        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div
                ...
                <div class="h-64 w-full bg-cover bg-center rounded-t-xl"
                    style="background-image: url('{{ asset('images/' . $order->product->image) }}')">
                </div>

                <!-- Product Details -->
                <div class="p-6">
                    <h3 class="text-2xl font-bold text-gray-900 mb-3">{{ $order->product->name }}</h3>
                    <p class="text-gray-700 text-base mb-4">{{ $order->product->description }}</p>

                    <div class="flex justify-between items-center mb-4">
                        <p class="text-lg font-semibold text-green-700">💲 {{ number_format($order->product->price) }}
                        </p>
                        <p class="text-sm text-gray-500">Your Order: {{ $order->quantity }}</p>
