                    ...
                    <form action="{{ route('product.update',$product->id) }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div>
                            ...
                            <input type="number" name="price" id="price" step="0.01"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                value="{{ceil($product->price)}}" placeholder="Enter price" required>
                            @error('price')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="file" name="image" id="image"
                                value="{{$product->image}}" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none">
                                <img src="{{asset('images/'.$product->image)}}" alt="" width="70px" height="70px">
                            @error('image')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
